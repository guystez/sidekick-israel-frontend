...
export default function DialogFeedBack({ open, onClose, handleCancel }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (!open) {
        onClose(); // Close the dialog after the specified time if it's not already open
      }
    }, 5000); // Adjust the time as needed (60000 milliseconds = 10 min)
