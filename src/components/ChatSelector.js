...
const ChatSelector = ({ selectedImage, onSend, onChangeImage, onLanguageCheck ,onLanguageSelection}) => {
  ...
  const handlesave = () => {
    axios.post('https://web-production-dd6e3.up.railway.app/date/check-hebrew', { termsAccepted: true, selectedSide, email: user.email })
      .then(response => {
        setOpen(false);
        onSend();
...
אנא לחץ על התמונה בצד המתאים. ולאחר מכן לחץ על ״שמור״.</b>
      <div style={{ padding: '20px', textAlign: 'center', position: 'relative' }}>
        ...
        {selectedSide === null && (
          <>
            <div className={`overlay ${autoSide === 'left' ? 'right-blurred' : 'left-blurred'}`}></div>
            <div className={`overlay ${autoSide === 'left' ? 'left-selected' : 'right-selected'}`}></div>
            <div className={`touch-container ${autoSide === 'left' ? 'touch-left' : 'touch-right'}`}>
        ...
        {selectedSide !== null && (
          <>
            <div className={`overlay ${selectedSide === 'left' ? 'right-blurred' : 'left-blurred'}`}></div>
            <div className={`overlay ${selectedSide === 'left' ? 'left-selected' : 'right-selected'}`}></div>
            <div className={`touch-container ${selectedSide === 'left' ? 'touch-left' : 'touch-right'}`}>
              <img src={TouchIcon} alt="Touch Icon" className="touch-icon" />
              <span className="touch-label">זה אני</span>
            </div>
          </>
